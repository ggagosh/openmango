...
impl CompletionMenuItem {
    ...
    fn icon_for_kind(kind: Option<CompletionItemKind>) -> IconName {
        match kind {
            ...
            | Some(CompletionItemKind::CONSTRUCTOR) => IconName::SquareTerminal,
            Some(CompletionItemKind::FIELD)
            | Some(CompletionItemKind::PROPERTY)
            | Some(CompletionItemKind::VARIABLE) => IconName::Folder,
            Some(CompletionItemKind::OPERATOR) => IconName::Asterisk,
            Some(CompletionItemKind::KEYWORD) => IconName::ALargeSmall,
            ...
            | Some(CompletionItemKind::INTERFACE) => IconName::LayoutDashboard,
            _ => IconName::SquareTerminal,
        }
    }

    fn kind_label(kind: Option<CompletionItemKind>) -> Option<&'static str> {
        match kind {
            Some(CompletionItemKind::METHOD)
            | Some(CompletionItemKind::FUNCTION)
            | Some(CompletionItemKind::CONSTRUCTOR) => Some("Method"),
            Some(CompletionItemKind::FIELD)
            | Some(CompletionItemKind::PROPERTY)
            | Some(CompletionItemKind::VARIABLE) => Some("Collection"),
        ...
        h_flex()
            ...
            .child(
                h_flex()
                    ...
                    .min_w(px(0.))
                    .child(
                        Icon::new(Self::icon_for_kind(item.kind))
                            .xsmall()
...
impl Render for CompletionMenu {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let overflow_right =
            (abs_pos.x + MAX_MENU_WIDTH) - window.bounds().size.width;
        let shift_left = if overflow_right > px(0.) {
            overflow_right.min(abs_pos.x)
        } else {
            px(0.)
        };
        let adjusted_pos = Point::new(pos.x - shift_left, pos.y);
        let adjusted_abs_x = abs_pos.x - shift_left;
        let max_width =
            MAX_MENU_WIDTH.min(window.bounds().size.width - adjusted_abs_x);
        let vertical_layout =
            adjusted_abs_x + MAX_MENU_WIDTH + POPOVER_GAP + MAX_MENU_WIDTH + POPOVER_GAP
