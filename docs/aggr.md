...
```
Documents | Indexes | Stats | Aggregation
```
...
| **Results** (bottom) | Document tree (read-only) showing output at the selected stage. |
...
Reuses the existing document collection view (document tree) in **read‑only** mode (same layout as `documents_view.rs`).
...
- No inline editing, save, delete, or bulk operations
- Expand/collapse, selection, and copy remain available
- Results are a preview only; they do not modify the collection
...
- Sample limit applied to all runs (configurable, default: 50)
...
### Phase 1 — State + Commands
- Add `PipelineState` + `PipelineStage` alongside `SessionData`.
- Build helpers for pipeline truncation (selected stage, enabled stages).
- Refactor aggregation commands to accept staged pipelines and update results + doc counts.

### Phase 2 — Three-Panel UI
- Stage list (left) with selection + enable/disable.
- Stage editor (right) with operator dropdown + JSON editor.
- Results panel (bottom) reusing document tree view (read-only).

### Phase 3 — Interactions
- Add/insert stage, duplicate, delete, move up/down.
- Keyboard shortcuts for list navigation and edits.

### Phase 4 — Analysis
- Implement explain + per-stage `$count` runs.
- Display badges + analysis view, plus warnings.
