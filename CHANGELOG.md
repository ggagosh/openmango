...
- Tasks: save any Transfer (export, import, copy) or Compare as a task with Save as task, then run it again from the Tasks tab (sidebar, or "Tasks" in the command palette) with Run now. A database comparison in sync mode saves as a sync task that covers every collection the source has, except the ones you unticked. Run now opens the connections a task needs, asks before a task writes, and runs through the same code as the tab it came from. Each run is recorded with its result, its counts per collection and a log, in an encrypted history of the last 100 runs per task. Edit opens a task in its tool, where Save task updates it
...
- Task recovery: comparison, sync and undo runs use connections of their own, so they never disturb the sidebar's. A dropped connection, a timeout or a primary stepping down is retried per collection after a growing, randomized wait, with a fresh connection, up to five times in fifteen minutes; each retry is logged in the run. Errors that won't pass, such as a missing permission, fail at once. A step with no progress for ten minutes is retried, and a run still going after 24 hours stops
- Task schedules: Schedule… in a task's details runs it by itself while OpenMango is open: every so many minutes or hours, daily or on weekdays, weekly on chosen days, or monthly, at a local time, with a calendar of the days it runs and the next three runs shown as you edit. A run missed while the computer slept or OpenMango was closed runs once, as a Catch-up, unless the next one is close. Runs go one at a time, and a task still running when it comes due again is skipped. Scheduled runs never ask: the safety limit stops them before writing, with Run anyway… in the run's details. A task that writes is approved for its connections as they are, so a changed connection stops its scheduled runs until you approve it again, and writing to a Production or protected connection on a schedule has to be allowed first. Scheduled exports name each file by its run and can keep only the newest 30. Pause keeps a schedule without running it
- Task problems and notifications: a scheduled task needs attention when its last run failed, the safety limit stopped it, a connection it uses changed or was deleted, or signing in failed. A failure that can pass, such as a server that can't be reached, needs attention only at the third in a row. The task's details say why, next to the button that fixes it, the Tasks list can show only the tasks that need attention, and the sidebar's Tasks button counts them. A failed scheduled run raises one notification per outage, with Open task, and the first run that works again says so; Schedule… can also announce every scheduled run that succeeds. A failed sign-in pauses the schedule until the connection is edited or it is resumed, so a wrong password isn't tried again and again. A run that can't reach its server when it starts now waits and tries again, like one that loses it midway
- Tasks can run while OpenMango is closed, on macOS 13 and later: turn on "Run even when OpenMango is closed" in Schedule…. A launch agent inside the app, listed in System Settings under Login Items, looks for due tasks about every 15 minutes, runs them without a window and records them in the task's history, so a run can start up to 15 minutes late. While OpenMango is open it runs its tasks itself, and the two never run the same task twice. A task set to run while closed needs attention when OpenMango is switched off in Login Items, with a button that opens it
- Tasks can run while OpenMango is closed on Windows too: the installed OpenMango adds a Task Scheduler task, "Run due tasks" in the OpenMango folder, when the first task turns on "Run even when OpenMango is closed", and removes it when the last one turns it off or OpenMango is uninstalled. It starts one minute past each quarter hour while you're signed in, runs on battery, and after a start missed while the computer slept or was off, starts as soon as it can. A task set to run while closed needs attention when the Task Scheduler task is disabled, with Open Task Scheduler
- Tasks can run while OpenMango is closed on Linux too, from the AppImage: OpenMango adds a systemd user timer, openmango-tasks.timer, when the first task turns on "Run even when OpenMango is closed", and removes it when the last one turns it off. It starts one minute past each quarter hour while you're signed in, and once after a start missed while the computer was off. While the keyring is locked a run doesn't start, since reading the passwords would ask to unlock it with nobody there; the tasks stay due. A disabled timer makes those tasks need attention, with Turn on. On every system, switching the runner back on outside OpenMango now clears that as soon as you come back to OpenMango
- Task notifications from the system: when a scheduled run fails, stops for a failed sign-in, works again, or succeeds and its task asks to hear about it, OpenMango also posts a system notification, on macOS, Windows and Linux. The app posts one only while you're not looking at it, with Open task; the background runner always posts one. Clicking one opens the task, and on macOS that also starts OpenMango if it's closed. macOS asks for permission the first time
- Task recovery for transfers: export, import and copy runs, and the counts their safety check reads, use connections of their own too. After a dropped connection or a timeout, an export starts over, and so does an import or copy that clears or drops its target first, with the same waits and limits. An import or copy that adds to what the target already has isn't run again, since that could write documents twice; the run log says so
