...
- Compare two databases: "Compare with…" on a database, or "Compare databases…" in the command palette, lists every collection of both databases paired by name, with estimated sizes, shows which exist on one side only, and opens any collection in its own comparison
